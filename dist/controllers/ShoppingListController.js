...
export class ShoppingListController extends BaseController {
    ...
    async create(req, res) {
        try {
            ...
            notificationService
                .sendToRefrigeratorMembers(groupId, `新採購清單「${listTitle}」出爐！`, `採買小隊報告！新清單已建立，快來看看需要買什麼！`, "shopping", {
                type: "shopping-list",
                payload: { refrigeratorId: groupId, listId: list.id },
            }, "official", userId, "list")
    ...
    async createItem(req, res) {
        try {
            ...
            const item = await shoppingListService.createItem(listId, userId, req.body);
            ...
            shoppingListService
                ...
                return notificationService.sendToRefrigeratorMembers(list.groupId, `${itemName} 加入採買行列！`, `採買小隊報告！${itemName} 已加入購物清單，收到請回報！`, "shopping", { type: "shopping-list", payload: { listId } }, "official", userId, "list");
            })
    ...
    async updateItem(req, res) {
        try {
            ...
            const item = await shoppingListService.updateItem(itemId, userId, req.body);
            ...
            if (item?.shoppingListId) {
                shoppingListService
                    ...
                    return notificationService.sendToRefrigeratorMembers(list.groupId, `「${listName}」清單內容變更！`, `採買小隊報告！「${listName}」已有異動，請各位確認！`, "shopping", { type: "shopping-list", payload: { listId: list.id } }, "official", userId, "list");
                })
