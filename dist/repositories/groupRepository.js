...
import { groups, groupMemberships, groupInvitations, users, } from "../db/schema/index.js";
...
export const v2GroupRepository = {
    async findGroupsByUserId(userId) {
        ...
        const groupsWithMembers = await db
            ...
            // User fields
            userId: users.id,
            userName: sql `COALESCE(${users.displayName}, '未知')`,
            userAvatar: sql `COALESCE(${users.profilePictureUrl}, ${users.avatar})`,
        })
            .from(groups)
            .innerJoin(groupMemberships, eq(groups.id, groupMemberships.groupId))
    ...
    async findGroupDetailById(groupId, userId) {
        ...
        const members = await db
            ...
            role: groupMemberships.role,
            joinedAt: groupMemberships.joinedAt,
            name: sql `COALESCE(${users.displayName}, '未知')`,
            avatar: sql `COALESCE(${users.profilePictureUrl}, ${users.avatar})`,
        })
            .from(groupMemberships)
            .innerJoin(users, eq(groupMemberships.userId, users.id))
            .where(eq(groupMemberships.groupId, groupId));
