...
async function migrateNotificationCategories() {
    ...
    try {
        ...
        for (const [type, category] of Object.entries(typeToCategory)) {
            const result = await query(`UPDATE notifications SET category = $1 WHERE type = $2 AND (category IS NULL OR category = 'system')`, [category, type]);
            console.log(`✅ 更新 type='${type}' → category='${category}' | 影響 ${result.rowCount} 筆`);
        }
        // 檢查結果
