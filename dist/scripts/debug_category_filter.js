...
async function debugCategory() {
    try {
      ...
      `);
        ...
        const { notifications } = await notificationService.getNotifications(userId, 1, 50, "stock");
        // 3. Analyze
        ...
        const offRes = await notificationService.getNotifications(userId, 1, 50, "official");
        console.log(`   Total Official Notifications: ${offRes.notifications.length}`);
    }
    catch (err) {
        console.error(err);
    }
