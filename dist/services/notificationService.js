...
export const notificationService = {
     ...
     */
    batchMarkAsRead: async (userId, ids, isRead = true) => {
        if (ids.length === 0)
     ...
     * 發送官方公告（廣播給所有使用者）
     */
    sendAnnouncement: async (title, message, type = "announcement", shouldPush = true, data = {}) => {
        ...
        const actionType = type === "release" ? "release_note" : "announcement_detail";
    ...
    `;
        ...
        console.log(`[Announcement] DB inserted ${insertResult.rowCount} notifications.`);
        ...
        if (shouldPush) {
            ...
            if (allTokens.length > 0) {
                console.log(`[Announcement] Broadcasting FCM to ${allTokens.length} devices.`);
                ...
                for (let i = 0; i < allTokens.length; i += batchSize) {
                    ...
                    try {
                        ...
                        console.log(`[Announcement] Batch ${Math.floor(i / batchSize) + 1} sent.`);
                    }
                    catch (err) {
