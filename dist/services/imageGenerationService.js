...
const getUnsplashImage = async (recipeName, category) => {
    try {
        ...
        if (accessKey) {
            const response = await fetch(`https://api.unsplash.com/photos/random?query=${encodeURIComponent(searchQuery)}&orientation=squarish&client_id=${accessKey}`);
            if (response.ok) {
        ...
        if (imageData) {
            try {
                ...
                if ("url" in imageData) {
                    ...
                    const uploadResult = await uploadToCloudinary(buffer);
                    console.log(`[ImageGen] Uploaded Gemini image to Cloudinary: ${uploadResult.secure_url}`);
                    return {
                        ...recipe,
