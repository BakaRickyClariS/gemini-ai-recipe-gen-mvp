...
router.get("/recipe/suggestions", (req, res) => controller.getSuggestions(req, res));
// GET /recipe/quota — 查詢剩餘次數（需認證）
router.get("/recipe/quota", jwtAuth, (req, res) => controller.getQuota(req, res));
// POST /recipe/stream — SSE 串流生成（Optional Auth）
router.post("/recipe/stream", optionalAuth, (req, res) => controller.stream(req, res));
// POST /recipe — 標準生成（Optional Auth）
router.post("/recipe", optionalAuth, (req, res) => controller.generate(req, res));
