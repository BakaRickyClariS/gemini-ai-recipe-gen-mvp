...
const requireDeploySecret = (req, res, next) => {
...
router.post("/announcements", requireAdmin, async (req, res) => {
    try {
        const { title, message, type = "announcement", pushNotification = true, data, } = req.body;
        if (!title || !message) {
            return res.status(400).json({ error: "title and message are required" });
        }
        const result = await notificationService.sendAnnouncement(title, message, type, pushNotification, data);
        res.json({
            success: true,
...
router.post("/announcements/release", requireDeploySecret, async (req, res) => {
    try {
        ...
        const shortMessage = lines.find((l) => l.trim().length > 0 && !l.startsWith("#")) ||
            "查看新功能與改善項目";
        const result = await notificationService.sendAnnouncement(title, shortMessage, "release", true, // 版本更新預設推播
        {
            version,
            releaseNotes,
            forceUpdate,
