...
export const getInventoryItems = async (
  ...
  if (status) {
    ...
    switch (status) {
      case "expired":
        ...
        break;
      case "expiring-soon":
        whereClause += ` AND expiry_date >= $${paramIndex} AND expiry_date <= $${
          paramIndex + 1
        }`;
  ...
  const listResult = await query<InventoryRow>(
    `SELECT * FROM inventory ${whereClause} ORDER BY created_at DESC LIMIT $${paramIndex} OFFSET $${
      paramIndex + 1
    }`,
    [...values, limit, offset]
  );
...
export const updateInventoryItem = async (
  ...
  for (const [key, dbField] of Object.entries(fieldMap)) {
    if (input[key as keyof UpdateInventoryInput] !== undefined) {
      ...
      values.push(
        input[key as keyof UpdateInventoryInput] as
          | string
          | number
          | boolean
          | null
  ...
  const result = await query<InventoryRow>(
    `UPDATE inventory SET ${updates.join(
      ", "
    )} WHERE id = $${paramIndex} AND refrigerator_id = $${
      paramIndex + 1
    } RETURNING *`,
    values
  );
...
export const updateInventorySettings = async (
  ...
  if (existing.rows[0]) {
    ...
    if (updates.length > 0) {
      ...
      const result = await query<SettingsRow>(
        `UPDATE inventory_settings SET ${updates.join(
          ", "
