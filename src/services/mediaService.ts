 ...
 */
export async function uploadToCloudinary(file: string | Buffer): Promise<any> {
  ...
  if (!hasApiKey && !uploadPreset) {
    throw new Error(
      "Cloudinary configuration missing: Must provide either API_KEY/SECRET or UPLOAD_PRESET.",
    );
  }
  ...
  if (!hasApiKey && uploadPreset) {
    ...
    if (typeof file !== "string") {
      throw new Error(
        "Unsigned upload only supports file paths. Please provide a file path string.",
      );
    }
