...
const validateCronSecret = (req: express.Request, res: express.Response, next: express.NextFunction) => {
...
router.post('/check-expiry', validateCronSecret, async (_req, res) => {
  try {
    ...
    // 按 user_id 分組
    const itemsByUser: Record<string, typeof expiringItems> = {};
    ...
    for (const [userId, items] of Object.entries(itemsByUser)) {
      ...
      await notificationService.send(
        ...
        'inventory',
        { type: 'inventory', payload: { refrigeratorId: items[0].refrigerator_id } }
      );
      notificationsSent++;
    }
