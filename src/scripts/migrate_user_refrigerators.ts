...
async function runMigration() {
  ...
  try {
    ...
    `);
    console.log(
      `   ✅ Backfilled ${settingsResult.rowCount} rows from settings.`
    );
    ...
    `);
    console.log(
      `   ✅ Backfilled ${inventoryResult.rowCount} rows from inventory usage.`
    );
    ...
    try {
      ...
      console.log(
        "   ℹ️  Skipping owner update (refrigerators table might not have owner_id yet)"
      );
    }
  } catch (err) {
