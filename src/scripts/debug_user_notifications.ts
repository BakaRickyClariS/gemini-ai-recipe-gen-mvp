...
async function debugNotifications() {
  try {
    ...
    if (userRes.rows.length === 0) {
      log(
        "⚠️ No user found with multiple refrigerators. Cannot verify cross-group view."
      );
    ...
    // Group by groupName
    const groupByFridge: Record<string, number> = {};
    notifications.forEach((n) => {
      const name =
        n.groupName || n.action?.payload?.refrigeratorId || "Unknown";
