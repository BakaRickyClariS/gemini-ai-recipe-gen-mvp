...
const inferSubType = (notification: { title: string; type: string }): string | null => {
  ...
  if (title.includes('到期') || title.includes('過期') || title.includes('庫存') || title.includes('低於')) return 'stock';
  ...
  if (title.includes('成員') || title.includes('加入') || title.includes('退出')) return 'member';
  if (title.includes('食譜') || title.includes('推薦') || title.includes('靈感')) return 'generate';
...
async function backfill() {
  ...
  try {
    ...
    const result = await query(
      `SELECT id, title, type FROM notifications WHERE sub_type IS NULL AND type != 'system'`
    );
    ...
    for (const notification of notifications) {
      const subType = inferSubType(notification as { title: string; type: string });
      
      if (subType) {
        await query(
          `UPDATE notifications SET sub_type = $1 WHERE id = $2`,
          [subType, notification.id]
        );
        updatedCount++;
    ...
    const statsResult = await query(
      `SELECT sub_type, COUNT(*) as count FROM notifications WHERE type != 'system' GROUP BY sub_type`
    );
