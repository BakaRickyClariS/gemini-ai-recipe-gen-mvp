...
async function testNotificationFlow() {
  ...
  try {
    ...
    if (all.total !== 2) throw new Error("Expected 2 notifications");

    // 3. Filter Check (Stock)
    const stocks = await notificationService.getNotifications(
      TEST_USER_ID,
      1,
      20,
      "stock"
    ...
    if (stocks.total !== 1) throw new Error("Expected 1 stock notification");
    if (stocks.notifications[0].category !== "stock")
      throw new Error("Category mismatch");

    // 4. Filter Check (Inspiration)
    const insights = await notificationService.getNotifications(
      TEST_USER_ID,
      1,
      20,
      "inspiration"
