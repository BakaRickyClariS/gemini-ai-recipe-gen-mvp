...
export class V2AiController extends BaseController {
  ...
  async generate(req: Request, res: Response): Promise<void> {
    try {
      ...
      const request: AIRecipeRequest = body;
  ...
  async stream(req: Request, res: Response): Promise<void> {
    try {
      ...
      const message =
        error instanceof Error ? error.message : "AI 服務暫時無法使用";
      const errorEvent = {
        id: `evt-${Date.now()}`,
   ...
   * 回傳預設 Prompt 建議（無需認證）
   */
  getSuggestions(_req: Request, res: Response): void {
   ...
   * 回傳當前使用者剩餘查詢次數（需認證）
   */
  getQuota(req: Request, res: Response): void {
    try {
      const userId = req.user?.userId;
      if (!userId) throw ApiError.unauthorized("請先登入");
