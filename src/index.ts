...
app.post('/api/v1/auth/login', async (req, res) => {
    try {
        let user = await ORM.UserModel.findOne({ where: { email: req.body.email } });
...
// #### User ####
app.get('/api/v1/user/:id', authenticateToken, async (req, res) => {
    try {
        const user = await ORM.UserModel.findByPk(req.params.id);
        res.json({
            status: true,
            message: "success",
            data: {
...
app.post('/api/v1/search/users', authenticateToken, async (req, res) => {
    try {
        const { keyword } = req.body;

        // 檢查關鍵字是否為空
        if (!keyword || keyword.trim() === '') {
          throw new Error("請提供關鍵字!");
        }

        const users = await ORM.UserModel.findAll({
          where: {
            [ORM.Op.or]: [
              { username: { [ORM.Op.iLike]: `%${keyword}%` } },
              { line_id: { [ORM.Op.iLike]: `%${keyword}%` } }
            ]
          },
          attributes: ['id', 'username', 'avatar', 'line_id'],
          limit: 50
        });
...
app.post('/api/v1/user', async (req, res) => {
    ...
    try {
        const user = await ORM.UserModel.findOne({ where: { email: req.body.email } });

        // 已經註冊過
        if (user) {
            throw new Error("The user has been register!");
        }

        // 建立使用者
        const createUser = await ORM.UserModel.create(req.body, { transaction: t });

        // 建立預設群組
        const createGroup = await ORM.GroupModel.create({
            name: "My Home",
            admin_id: createUser.id
        }, { transaction: t });
...
app.get('/api/v1/user/:id/groups', authenticateToken, async (req, res) => {
    try {
        const userWithGroupsAndMembers = await ORM.UserModel.findOne({
          where: { id: req.params.id },
          attributes: [],
          include: [
            {
              model: ORM.GroupModel,
              as: 'groups',
              through: {
                attributes: []
              },
...
// #### Group ####
app.get('/api/v1/group/:id', authenticateToken, async (req, res) => {
    try {
        const groupWithMembers = await ORM.GroupModel.findOne({
          where: { id: req.params.id },
          include: [
                {
                  model: ORM.UserModel,
...
app.post('/api/v1/group/:groupId/user/:userId', authenticateToken, async (req, res) => {
    try {
        await ORM.GroupUserModel.create({
...
app.delete('/api/v1/group/:groupId/user/:userId', authenticateToken, async (req, res) => {
    try {
        const group = await ORM.GroupModel.findByPk(req.params.groupId);
        if (!group) {
          throw new Error("找不到群組");
        }

        if (group.admin_id == req.params.userId) {
          throw new Error("組群管理員不能刪除自己");
        }
