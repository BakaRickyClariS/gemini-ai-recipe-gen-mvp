...
前端反映在切換通知分類 Tab 時，部分通知（如 `group` 群組加入、`shopping` 採買清單）會莫名消失。

- **問題原因**: 後端為了迎合舊版邏輯與資料庫庫存，而在建立通知時強制把群組與系統推播都歸類成了 `official`，導致前端傳送 `?category=stock` 時無法正確取得這些通知。
- **後端修復**:
  1. 修正 V2 API 接到 `?category=stock` 與 `?category=official` 時的資料庫查詢過濾邏輯。後端現在會根據 `type` 與是否含有群組特徵（`group_name`）等真實業務條件進行分類回傳。
  2. 確保此改動完全 **不影響 V1 API** 原有的行為。
  3. 修正建立通知時的分類寫入邏輯，使其在未來更加精確。
- **前端 Action / 對接方式**:
  - **請維持簡潔語意化的 API 請求**：
    - 取得食材管家通知：`GET /api/v2/notifications?category=stock`
    - 取得官方公告通知：`GET /api/v2/notifications?category=official`
  - **移除前端防呆與手動補丁**: 請撤除前端目前針對通知資料所做的「全域拉取加本地防呆過濾」這類的暫時性措施 (Workarounds)。現在您可以安心、直接地把後端傳回的資料丟給畫面渲染，讓分頁機制順暢運作！
